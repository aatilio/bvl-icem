...
% --- MACRO PARA VARIABLE PRINCIPAL (abreviada en ecuaciones, nombre completo en Tabla 1) ---
\newcommand{\semcert}{\text{Semillas}}
...
\begin{equation}
	\ln(\text{Rend.}_i) = \beta_0 + \beta_1 \, \semcert_i + X_i \cdot \hat{\Gamma} + \varepsilon_i
\end{equation}
...
\footnotesize
\begin{split}
    \ln(\text{Rend.})_i &= \beta_0 + \beta_1\,\texttt{semillas\_cert.}_i + \beta_2\,\texttt{fuente\_agua}_i \\
    &\quad + \beta_3\,\texttt{sequia}_i + \beta_4\,\texttt{lluvias\_dest.}_i \\
    &\quad + \beta_5\,\texttt{plagas\_enf.}_i + \beta_6\,\texttt{otros\_fact.}_i \\
    &\quad + \beta_7\,\texttt{mujer\_prod.}_i + \beta_8\,\texttt{educ\_sup.}_i \\
    &\quad + \beta_9\,\texttt{edad\_prod.}_i + \hat{\mu}_d + \varepsilon_i
\end{split}
\end{equation}
...
\begin{equation}
\ln(\text{Rend.}_i) = 8{,}2564 + 0{,}0982 \, \semcert_i + X_i \cdot \hat{\Gamma} + \varepsilon_i
\end{equation}
donde $\semcert_i$ (\texttt{semillas\_certificadas}) es la variable dicotómica de adopción ($X_{1i}$) y $X_i \cdot \hat{\Gamma}$ representa el vector estimado que engloba tanto las variables observables de control a nivel individual como los efectos fijos espaciales departamentales ($\hat{\mu}_d$).
...
\begin{equation}
\begin{split}
\ln(\text{Rend.}_i) &= 8{,}2564 + 0{,}0982 \, \semcert_i \\
